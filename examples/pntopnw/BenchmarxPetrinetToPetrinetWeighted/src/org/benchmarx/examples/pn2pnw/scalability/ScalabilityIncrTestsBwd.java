...
public class ScalabilityIncrTestsBwd extends ScalabilityTests {
	...
	private void createOneMorePTPUnit(int nrOfUnits) {
		...
		timer.prepareTargetEditAfterSetUp(trgEdit(helperPnw::renameToLettersAndDigits, () -> helperPnw.createNPTPUnits(nrOfUnits, "P")));
		try {
			assertTimeoutPreemptively(Duration.ofSeconds(TIMEOUT), () -> {
				recordResult(tool, nrOfUnits,
						timer.measureTargetEditAfterSetUpInS(
						trgEdit(() -> helperPnw.createNPTPUnits(1, "X"))));
			});
		} catch (Throwable t) {
