...
public class ScalabilityIncrTestsFwd extends ScalabilityTests {
	...
	private void createOneMoreActivity(int nrOfActivities) {
		...
		timer.prepareSourceEditAfterSetUp(srcEdit(() -> helperGantt.createNActivities(nrOfActivities, "A")));
		try {
			assertTimeoutPreemptively(Duration.ofSeconds(TIMEOUT), () -> {
				recordResult(tool, nrOfActivities,
						timer.measureSourceEditAfterSetUpInS(
						srcEdit(() -> helperGantt.createNActivities(1, "X"))));
			});
		} catch (Throwable t) {
	...
	@ParameterizedTest @MethodSource("tools")
	public void testCreate0000300Activities(BXTool<GanttDiagram, CPMNetwork, Decisions> tool) { this.tool = tool; initialise(); createOneMoreActivity(300); }
